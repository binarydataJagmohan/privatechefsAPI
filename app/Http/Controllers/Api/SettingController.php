...
class SettingController extends Controller
{
    ...
    {
        try {
            $data = Setting::where('id', $request->id)->where('status', 'active')->first();
            return response()->json([
                'status' => true,
                'data' => $data
            ], 200);
    ...
    {
        try {
            $receipt = Setting::find($request->id);
    ...
    {
        try {
            $data = Setting::where('slug', $request->slug)->where('status', 'active')->first();
            return response()->json([
                'status' => true,
                'data' => $data
            ], 200);
